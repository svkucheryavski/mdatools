...
#'
#' @description
#' Saves PLS results to CSV file with structure identical to
#' the one generated by web-application at https://mda.tools/pls
#'
#' @param res
#' PLS results (object of class \code{plsres})
#' @param fileName
...
writeCSV.plsres <- function(res, fileName, name, sep = ",", dataFile = "", ...) {
   ...
   rowLabels <- rownames(res$xdecomp$scores)

   # number of rows with/without outliers
   nAll <- nrow(res$xdecomp$scores)
   ...
   addChunk <- function (name, values, out) {
      ...
      if (!is.null(name)) {
         out <- c(out, paste1(name, ': ', sep, filler))
         out <- c(out, paste1('object', sep, compNames))
      }
   ...
   if (length(outliers) > 0) {
      out <- c(out, paste1('Removed outliers:', sep, length(outliers), filler))
   }

   if (length(exclvars) > 0) {
      out <- c(out, paste1('Excluded variables:', sep, length(exclvars), filler))
   }
   out <- c(out, ' ')

   # variances and predictions
   if (nResp > 0) {

      out <- c(out, paste1('Variances:', sep, filler))
      out <- c(out, paste1('', sep, compNames))
      out <- c(out, paste1('xexpvar:', sep, paste1(res$xdecomp$expvar/100, sep)))
      out <- c(out, paste1('xcumexpvar', sep, paste1(res$xdecomp$cumexpvar/100, sep)))

      out <- c(out, paste1('yexpvar:', sep, paste1(res$ydecomp$expvar/100, sep)))
      out <- c(out, paste1('ycumexpvar:', sep, paste1(res$ydecomp$cumexpvar/100, sep)))


      for (ny in seq_len(nResp)) {
         out <- c(out, ' ')
         out <- c(out, ' ')

         out <- c(out, paste1('Response variable:', sep, dimnames(res$y.pred)[[3]][ny], filler))
         out <- c(out, ' ')

         out <- c(out, paste1('FoMs:', sep, compNames))
         out <- c(out, paste1('RMSE', sep, paste1(res$rmse[ny, ], sep)))
         out <- c(out, paste1('R2', sep, paste1(res$r2[ny, ], sep)))
         out <- c(out, paste1('Slope', sep, paste1(res$slope[ny, ], sep)))
         out <- c(out, paste1('Bias', sep, paste1(res$bias[ny, ], sep)))

         out <- c(out, ' ')
         out <- c(out, paste1('Predictions:', sep, compNames))
         out <- addChunk(NULL, res$y.pred[, , ny], out)
      }
   } else {
      nResPred <- dim(res$y.pred)[3]
      for (ny in seq_len(nResPred)) {
         out <- c(out, paste1('Response variable:', sep, dimnames(res$y.pred)[[3]][ny], filler))
         out <- c(out, ' ')
         out <- c(out, paste1('Predictions:', sep, compNames))
         out <- addChunk(NULL, res$y.pred[, , ny], out)
   ...
   out <- c(out, ' ')
   out <- addChunk('X-scores', res$xdecomp$scores, out)
   out <- c(out, ' ')
   out <- addChunk('Score distances, h', res$xdecomp$T2, out)
   out <- c(out, ' ')
   out <- addChunk('Orthogonal distances, q', res$xdecomp$Q, out)

   if (!is.null(res$y.ref)) {
      out <- c(out, ' ')
      out <- addChunk('Y-scores', res$ydecomp$scores, out);
      out <- c(out, ' ')
      out <- addChunk('Orthogonal distances, z', res$ydecomp$Q, out);
   }
